...
- **Worker idle timeout (#466)**: migration **076** `agent_profiles.idle_timeout_ms`(NULL=기존 30분), `runs.terminal_reason`. health loop은 payload 없는 poll heartbeat를 activity로 보지 않고 SQLite 시각을 UTC로 해석한다. idle timeout 뒤 종료된 run은 기존 terminal status를 유지하면서 `terminal_reason='idle_timeout'`으로 구분한다. 결론 출력 뒤 process가 남은 운영 3건의 조기 종료 원인은 코드만으로 특정하지 못해 heuristic 종료는 추가하지 않았다. 조사: `docs/worker-idle-timeout-investigation-2026-07-29.md`.
...
- Memory 주입은 turn context 기준: codebase-specific turn 은 해당 Workspace(+profile+user), generic turn 은 profile+user + watch-list 요약, auto-review turn 은 worker codebase Workspace 만. ledger 는 실제 선택 주입된 owner 만 기록한다. **✅ codebase-pool-memory-axes-brief 전 구현 완료 (2026-07-20, B0~flip merged)**: profile 축 + watch-list synthetic owner(edgeless, revision=watchlist_version) 를 turnMode 별로 `conversationService` operator 블록에서 합성(B1). `PALANTIR_MEMORY_MULTI_OWNER`/`PALANTIR_MEMORY_DISTILL` **기본 ON**(DISTILL 은 ANTHROPIC_API_KEY 있을 때만 스케줄러). 관련: 모든 operator_instance 가 정확히 1 private profile 소유(B0, `profile_id` NOT NULL) / profile 전용 revision(B2a, migration 065) / profile distill owner-generic(B2b) / R6 fact admission gate(B-adm, node_source='project'만 승격). watch-list 요약은 project.name 을 detectInjection/redactSecrets 로 살균, injection 이면 id-only.
...
- **노드 인식 DirectoryPicker (task_85d43f96)**: 프로젝트 폼은 **실행 노드를 먼저** 고르고 그 노드의 파일시스템을 탐색한다. `/api/fs?nodeId=` 는 `local`/미지정이면 control-plane(`storage.fsRoot`), 그 외에는 node executor 의 `listDirectoryEntries` 로 `exposed_roots` 안만 — 양쪽 다 realpath 정규화 후 containment 재검사(root 안에 살면서 밖을 가리키는 symlink 를 fail-closed `symlink_escape` 로 거부). 실패는 항상 `reason` 을 실어 보낸다(`fsService.BROWSE_REASONS`), 그리고 **`nodeBindingValidator` 의 저장 시점 거부도 같은 vocabulary 를 import 해서 쓴다** — 새 실패 케이스를 추가하면 양쪽과 `DIRECTORY_PICKER_LABELS.errorReasons` 를 함께 갱신할 것.
- **`apiFetch({ allowAppForbidden })` 는 opt-in**: `/api/fs` 는 "exposed_roots 밖" / "권한 없음" 을 **403** 으로 답한다. `apiFetch` 는 기본적으로 401/403 을 로그인 만료로 보고 `/login.html` 로 bounce 하므로, opt-in 없이는 경로 오타 하나에 조작자가 로그인 페이지로 튕기고 DirectoryPicker 의 node-change reset(가장 흔한 두 reason 이 정확히 그 403 들)이 **영영 실행되지 않는다**. 판별자는 **403 응답의 non-empty string `reason`** — auth 미들웨어는 라우트 진입 전에 거부하므로 auth 403 은 절대 `reason` 을 싣지 않는다. **`reason` 없거나 빈 403 및 모든 401 은 opt-in 상태에서도 계속 bounce** 한다(진짜 인증 실패). `api-fetch-client.test.js` 가 이 경계를 고정.
