...
- **worker tier 토큰 refuse**: codex worker 의 args_template 에 `service_tier`/`features.fast_mode` 토큰 있으면 fail-closed(`worker:tier_forbidden` + throw). `baseArgs`(템플릿)만 스캔 — 강제 `-c service_tier="default"`(extraArgs)는 미스캔. UI: `ModelPoliciesView`(#resources/models 서브탭, tri-state 편집 + effective-source 미리보기, cookie PUT/409 refetch). a11y/visual 게이트에 resources/models 추가.
