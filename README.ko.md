...
K-2 PR #150~#153 에서 라이트 모드 launch 완료. NavSidebar 하단 토글은 3-state — ◑ system (`prefers-color-scheme` 자동) / ☀ light / ☽ dark — 선택은 `localStorage.palantir.theme` 에 저장. `server/public/theme-init.js` 가 `<head>` 에서 `tokens.css` 보다 먼저 로드되어 `<html data-theme>` 설정 + `<meta name="theme-color">` 동적 갱신 → FOUC 없음, 모바일 chrome 색상도 활성 테마 추종.
...
```
GET    /api/mcp-server-templates       — 목록
POST   /api/mcp-server-templates       — 생성 { alias, command, args?, env?, allowed_env_keys? }
GET    /api/mcp-server-templates/:id   — 조회
PATCH  /api/mcp-server-templates/:id   — 수정 (`updated_at` 자동 bump → RunInspector 가 preset snapshot 대비 drift 표시)
DELETE /api/mcp-server-templates/:id   — 삭제 (preset 이 참조 중이면 거부)
```
