...
- **남은 것(선택, Trigger-wait)**: **Phase 2** = Worker 구조화(agent_profiles→args_template 결정적 치환) + operator_profile 스코프 + claude thinking + gemini. **Phase 3** = Node feasibility 검증(actionable error) + cost cap constraint.
