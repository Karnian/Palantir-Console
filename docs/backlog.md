...
- **진행**: **✅ G1 완료·merged** (migration 054 `tasks.goal_enabled/goal_max_attempts` + `runs.goal_report/final_output` + `goalPrompt.compileGoalPrompt`[결정적, forward-compat verify/attempt 훅] + `goalReport.parseGoalReport`[fenced-block, never-throws, last-wins] + spawnQueuedRun goal-prompt 주입[non-goal 불변] + captureGoalOutput[run terminal, harvestService-독립, file-backed tee §5k-2: SubprocessEngine write-stream/TmuxEngine pipe-pane, getOutput fallback, annotate-only] + 테스트 28종). codex 최종 R1 FAIL(1B harvestService 결합+3S)→R2 FAIL(2S UTF-8 tail/tee close)→**R3 PASS**. G2~G5 다음.
