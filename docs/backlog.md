...
- **남음 (다음 세션, handoff `docs/handoff-memory-layer-pr1-2c.md`)**: remember(R4 쿠키=active/PM=candidate, auth.method 분리) + PR4(UI 사후교정) + PR5(안전·decay·graceful shutdown) + PR3c(fuzzy 병합·cross-run confidence, optional). spec `docs/specs/memory-layer-brief.md` §10.
