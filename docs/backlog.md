...
- **K-5-followup 모달 visual** ✅ 부분완료(2026-07-13, #369, **codex-goal 위임**). Command Palette(Ctrl+K)+New Agent modal(#agents "새 에이전트") 열린 상태 4 시나리오(dark/light×desktop). element-scoped 다이얼로그 스냅샷+`toBeVisible()` 계약. **codex-goal 스킬로 격리 worktree 위임구현→호스트 외부검증(결정성 2회+baseline 육안+Themis PASS)**. visual 64/64. 드로어(데이터 필요)+나머지 모달 후속.
