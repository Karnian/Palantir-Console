...
### ~~MP. Model/Effort 정책 레이어 (설정 페이지)~~ ✅ Phase 1 완료·머지 (2026-07-16)
- **Spec**: [`docs/specs/model-policy-brief.md`](./specs/model-policy-brief.md) (LOCKED, Codex 5R GO + 사용자 lock-in). **Phase 1 전 구현 완료** — MP-1~MP-5 각각 codex-goal 위임 + Claude 교차리뷰 + 전체 회귀 게이트(2331 tests) + a11y/visual 게이트.
- **완료**: migration **061**(model_policies scope↔scope_id CHECK + JSON-object CHECK + per-vendor UNIQUE + updated_at bump trigger, model_policy_audit append-only, AFTER DELETE ON projects orphan trigger, runs.session_model/session_effort) + `modelPolicyResolver`(순수 필드단위 tri-state inherit/explicit/`__cli_default__`, F-1 short-circuit) + `modelPolicyService`(단일 tx CAS 409/audit/project-exists, resolveEffective, resolveServiceTier standard→default 매핑) + `routes/modelPolicies`(cookie-only + Origin CSRF) + spawn 배선(Top/Operator model·effort·tier, `runs` 스냅샷 persist + boot resume 재사용, codexAdapter reasoning_effort emit) + worker args_template tier-token fail-closed refuse + `ModelPoliciesView`(#resources/models 서브탭, tri-state UI, effective-source 미리보기). **빈 정책 = byte-identical**(기존 spawn/resume 스위트 무변경). 축 = Role×Vendor + per-Codebase(operator). tier=live(fast_mode 3-state) / model·effort=session-snapshot.
- **Phase 2 Worker 구조화 ✅ 완료·머지 (2026-07-16)**: migration 062(agent_profiles.model/reasoning_effort, NULL=byte-identical) + agentVendor 공용 resolver + tokenizer 저장검증(merged-state PATCH, `-m` 오탐 방지) + vendor-aware 주입(codex extraArgs / claude spec.model, F-1 tier 불변) + claim-전 non-retryable backstop + runs 스냅샷 + AgentsView 구조화 필드. P2-A1/A2/C 각 codex-goal + Claude 리뷰 + 회귀 2344 + a11y/visual 무회귀. Codex 설계 3R GO.
- **Phase 2 나머지 3개 DROP/DEFER (소비자 부재 = dead code)**: claude thinking(CLI 플래그 없음), gemini vendor(manager 어댑터 없음), operator_profile 스코프(specialist=non-CLI backend). **Phase 3(선택)** = Node feasibility 검증 + cost cap constraint.
...
- **K-5-followup 모달/드로어 visual** ✅ 부분완료(2026-07-13, **codex-goal 위임**). **모달 #369**: Command Palette(Ctrl+K)+New Agent modal(#agents "새 에이전트") 4 시나리오. **드로어 #370**: DriftDrawer 열린 상태 2 시나리오 — data-gated 라 `page.route` 로 drift API 를 고정 fixture mock→트리거 노출→열기, `.drift-row-time` mask. 공통 element-scoped 다이얼로그 스냅샷+`toBeVisible()` 계약. **RunInspector #371**: 완료 run 슬라이드오버 기본 Output 탭 2 시나리오 — `**/api/runs**` catch-all(URL dispatch)+nodes/projects mock 으로 completed-run fixture 주입, `.run-status-started` mask, `toContainText` 로 채워짐 검증. **codex-goal 스킬로 격리 worktree 위임구현→호스트 외부검증(결정성 2회+baseline 육안+Themis PASS)**. **폼 모달 #372**: 나머지 결정적 오픈 폼 모달 3종(새 코드베이스/새 MCP 서버/새 작업) 6 시나리오 — 헤더 `+ New` 버튼→열기, `Modal` primitive(aria-labelledby dialog 타깃), mock/mask 불요. visual 74/74. **모달/드로어/인스펙터 결정적 오픈 커버리지 완결**(남은 건 성능 회귀=별도 phase).
