...
| 13 | **OP 3단계 스케줄러 파리티** / **4단계 얇은 CLI** ★다음 | 동 brief §4 | 🔨 **A1 merged** (#514, `operator_schedules` grace window + misfire policy). **#12 보류로 이 트랙이 다음 순위** — 사용자가 Orca 로 실제 무인 운영하는 축이 cron 기반 에이전트 턴이라 Orca 대체의 실질이 여기 있다. 남음: precheck(named verify check 참조, raw command 금지)·프롬프트 외부화/버전관리·action target / 얇은 CLI(spawn·follow·input·cancel 만) | PR 2~3개 |
