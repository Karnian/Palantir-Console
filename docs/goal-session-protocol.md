...
| 12 | **OP 2단계 Action control plane** ★본체 | `docs/specs/action-control-plane-brief.md` (LOCKED, codex 4R GO + 사용자 lock-in 2026-08-05) | 🔨 **PR1·PR3 merged, PR2/PR4 남음.** spec lock (#508) → PR1a ledger 기반 (#509) → PR1b-1 forward execution broker + fake GitHub gateway (#510) → PR1b-2a read-only reconcile + orphan recovery (#511) → PR1b-2b bounded repair sub-machine (#512) → PR3 read-only `#actions` 관측 UI (#513). **남음: PR2 = 실 GitHub gateway (외부 side effect + 격리 시험 repo 실검증 → 착수 전 사용자 확인 필요)**, **PR4 = task `goal_kind='action'` 배선 + Operator 가 action 선언** (fake gateway 로 검증 가능, PR2 비의존). | 다중 PR |
| 13 | **OP 3단계 스케줄러 파리티** / **4단계 얇은 CLI** | 동 brief §4 | 🔨 **A1 merged** (#514, `operator_schedules` grace window + misfire policy). 남음 — precheck(named verify check 참조, raw command 금지)·프롬프트 외부화/버전관리·action target / 얇은 CLI(spawn·follow·input·cancel 만) | PR 2~3개 |
