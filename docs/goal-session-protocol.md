...
| 13 | **OP 3단계 스케줄러 파리티** / **4단계 얇은 CLI** ★다음 | 동 brief §4 | 🔨 **A1 merged** (#514, `operator_schedules` grace window + misfire policy, migration 088). **#12 보류로 이 트랙이 다음 순위** — 사용자가 Orca 로 실제 무인 운영하는 축이 cron 기반 에이전트 턴이라 Orca 대체의 실질이 여기 있다. **A2 precheck 은 착수 전 #524 의 설계 계약을 먼저 닫아야 한다** (codex NO-GO: BLOCKER 5 — 평가 시점이 delivery 면 너무 늦음/terminal status 미정의(`waiting_reason` 은 자유 TEXT)/active-invocation 단일성 충돌/daily cap 계약/원격 runner 영구 부재. + SERIOUS 7 — kind 변경·project 변경 우회면 등). 이후 A3 프롬프트 외부화·버전관리 / 얇은 CLI(spawn·follow·input·cancel 만). **action target 은 영구 제외**(#522). | PR 2~3개 |
