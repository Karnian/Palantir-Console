...
| 12 | **OP 2단계 Action control plane** ★본체 | `docs/specs/action-control-plane-brief.md` (LOCKED, codex 4R GO + 사용자 lock-in 2026-08-05) | 🔨 **설계 lock, PR1 구현 중.** 첫 슬라이스 GitHub create_issue: `action` goal kind + ledger + `(task_id,slot)` idempotency + read-back + `unknown` read-only reconcile(at-most-one 자동 POST) + durable 승인 gate(cookie, approved_params_hash) + attempt fencing. PR1=fake gateway 전 경로(외부 0) → PR2 실 GitHub → PR3 UI → PR4 goal_kind 배선 | 다중 PR |
