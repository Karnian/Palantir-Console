...
- #3 K-2 시각 회귀 자동화 (Playwright screenshot diff)
...
후속 후보 (별도 phase):
- **K-4-followup-contrast** — baseline waiver 30 row 의 surface 별 fix (PR 다수, 14일 만료 압력)
- **K-4-followup-card-markup** — `.agent-card` / `.project-card` 의 nested-interactive 구조 재설계
- **#3 K-2 시각 회귀 자동화** (Playwright screenshot diff) — 남은 마지막 후보
