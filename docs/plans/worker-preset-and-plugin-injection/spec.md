...
### ~~US-007~~ — 제외 (Phase 10G 종결)
> argv-level Tier 2 wiring 검증으로 G1 보장 충분 → 자동화 시나리오에서 제외. 정식 spec 은 docs/specs/worker-preset-and-plugin-injection.md 참조.
...
2. Else if .claude-auth.json has valid CLAUDE_CODE_OAUTH_TOKEN
   → inject CLAUDE_CODE_OAUTH_TOKEN env with that token value
3. Else if macOS keychain has valid token
   (`security find-generic-password -s <service> -w` 로 값 추출)
   → inject CLAUDE_CODE_OAUTH_TOKEN env
4. Else → fail-closed: 400 "Isolated preset requires Claude auth.
...
- [x] US-001 ~ US-006, US-008 자동화 테스트 (US-007 제외)
