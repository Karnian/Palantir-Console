...
> **상태**: v4 **LOCKED** → **🎉 전 구현 완료** (2026-07-20). Codex R1 NO-GO→v2, R2 NO-GO→v3, **R3 REVISE→조건부 GO 충족**. 구현: A0→P0→A1→A2a→A2b (A-트랙 merged, ~a77c0b7) → **B-트랙 전부 merged** (B0 25c7d06 / B2a 7550d05 / B1 dc2a3f9 / B2b a0b0176 / B-adm 9eee7a1 / 통합-fix eca0392 / flip 02e149d), phase 별 codex 교차검증(계획 NO-GO→반영→codex-goal 위임→호스트 npm test→Claude 리뷰). **최종 통합리뷰**: SERIOUS(profile-delete FK 루프) 수정 + BLOCKER(B-adm staleness) = migration 066 오염정리 + payload 확장 follow-up 문서화. **default rollout: `PALANTIR_MEMORY_MULTI_OWNER`/`PALANTIR_MEMORY_DISTILL` 기본 ON** (사용자 결정 2026-07-20; DISTILL 은 ANTHROPIC_API_KEY 있을 때만 스케줄러 기동). npm test 2406 그린. **B-adm staleness 근본해결 ✅** (ccfa153, 2026-07-20): harvest payload `declared_node_major` 3-way 관측(`classifyDeclaredNode` exact/none/indeterminate — 오직 "유효 dir + 선언 부재"만 none 철회, malformed root·engines·빈 선언·비정상/소멸 루트[dev/ino 재검증]·range·remote 는 indeterminate no-op) → `memoryService.retractR6Fact`(한 tx archive+revision bump, origin='rule:R6' AND pinned=0 guard) → `createR6FactCapture` declared 기반 admission(none→retract / exact→promote / legacy 부재→기존 node_source gate byte-identical). codex 계획 NO-GO + 최종 diff 적대 4R(malformed→비정상루트→TOCTOU 오철회 경로 순차 폐색)→GO. npm test 2437 그린. **공유 detectInjection/redactSecrets hardening ✅** (d14d188, 2026-07-20): 9개 모듈이 공유하는 `memorySanitize.js` 강화 — (1)`normalizeForScan`(NFKC + zero-width/bidi/제어 strip, 개행 보존) → `detectInjection` 이 raw ∪ normalized 양쪽 스캔(순수 additive, fullwidth/zero-width 회피 차단, 출력 불변) (2)한국어 인젝션 패턴(지시무시=ignore+action 2-branch 하다-verb/보조용언, 역할주입=identity+copula, role marker — 명사 anchored·인용/부정/회고 배제로 human-remember 400 오탐 0) (3)provider secret 추가(Google AIza/ya29, Stripe, npm_, glpat-, conn-string user:pass@; 고정길이=suffix-agnostic, 가변길이=negative-lookahead 경계) (4)`Basic` auth = **바이트레벨 charset-agnostic 검증**(정규식 아님 — decode 미선언 charset 검증은 decidable 하지 않음: C0/DEL 거부[RFC 7617 control 금지] + colon 0x3A 요구, 高바이트 허용 → UTF-8/ISO-8859-1/Windows-1252/Shift-JIS/EUC-* 커버, stateful ESC charset[ISO-2022-*/HZ]은 명시적 accepted-debt) + obfuscation backstop(normalizeForScan 후 재검사 fail-closed) (5)REDACTION_VERSION 1→2. **codex 적대검토 13R**(계획 GO-WITH-FIXES → diff R1 NO-GO[혼합 obfuscated 누출·human-400 오탐 2 BLOCKER] → R2~R12 Basic 경로 순차 폐색[디코딩·padding·floor·빈필드·charset] → R13 GO[accepted-debt scope 인정]). 한국어 recall 완전성은 accepted-debt NIT. 직렬 npm test 2451 그린. **교훈: 미선언-charset credential 검증은 undecidable — codex 가 utf8/latin1/byte-level 각 접근의 charset gap 을 순차 입증, 명시적 scope-out 이 수렴점. Basic 은 Bearer 있음에도 R4~R12(9라운드) 소모 = footgun, 바이트레벨이 근본해. codex-goal 세션훅 auto-commit 함정 재확인(base 기준 patch).**
