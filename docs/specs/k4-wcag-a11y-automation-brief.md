...
- `critical` / `serious` / `moderate` → **fail** (gate)
- `minor` → **report-only**.

**업데이트 (2026-07-13, 사용자 lock-in — PR #367)**: `moderate` 를 report-only → **fail 로 승격**. 근거: K-4 launch 후 cleanup 이 진행돼 56 시나리오 전부 moderate 위반 0 개에 도달 → 정리 부담(원래 L→XL 우려) 없이 게이트만 조인다. 이제 신규 moderate 회귀는 fix-or-waive. waiver 는 non-contrast 룰과 동일(≤30d transitional). launch 시점의 원안(아래)은 히스토리로 보존:
...
3. ~~**moderate severity 자동 fail 로 승격**~~ ✅ **완료 (2026-07-13, #367)** — moderate 위반 0 도달 후 fail 승격. `FAIL_IMPACTS = {critical, serious, moderate}`, minor 만 report-only.
