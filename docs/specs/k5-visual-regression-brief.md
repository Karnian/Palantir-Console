...
2. ~~**모달 / 드로어 / 오버레이 시각 검증**~~ — **부분완료**: **모달 (#369)** = 두 모달(Command Palette=`Ctrl+K`, New Agent modal=`#agents` "새 에이전트" 버튼) 열린 상태 dark/light × desktop 4 시나리오. **드로어 (#370)** = **DriftDrawer** 열린 상태 dark/light 2 시나리오. 드로어는 **data-gated**(트리거가 drift 있을 때만 노출)+fresh empty DB 라 `page.route('**/api/dispatch-audit*')` 로 drift API 를 **고정 fixture 로 mock**→트리거(`드리프트 경고` 칩) 노출→열기. **element-scoped `.drift-drawer` 스크린샷** + 유일 동적 필드 `.drift-row-time`(상대시각) **mask**. **RunInspector (#371)** = 완료 run 슬라이드오버(`.run-inspector-slideover`) 기본 Output 탭 dark/light 2 시나리오 — data-gated(`#run/:id` 라우트+per-run 폴링)라 `**/api/runs**` 단일 catch-all(URL dispatch: list/detail/output/events)+nodes/summary·projects mock 으로 고정 completed-run fixture 주입, `.run-status-started`(상대시각) mask, `toContainText(task_title)` 로 실제 채워짐 검증. 공통: `toBeVisible()` 계약(열기 실패 loud fail). **폼 모달 (#372)** = 나머지 결정적 오픈 폼 모달 3종(새 코드베이스=#operator/codebases, 새 MCP 서버=#resources/mcp-servers, 새 작업=#board) 헤더 `+ New` 버튼→열기, dark/light 6 시나리오. 모두 `Modal` primitive(aria-labelledby 로 dialog 타깃), 폼이라 mock/mask 불요. 넷 다 codex-goal 위임 구현 + 호스트 외부검증(결정성 2회 + baseline 육안 + Themis PASS).
   - **오퍼레이터 로스터 populated 상태 (#396)**: fresh empty DB의 EmptyState baseline은 유지하고, deterministic API fixture로 Configured/Live/Available 카드가 모두 보이는 전체 화면 dark/light × desktop/mobile 4개와 Configured 상세 모달 dark/light 2개를 추가한다. 카드 밀도와 카드→상세 팝업 정보 이동을 실제 픽셀 회귀 대상으로 고정한다.
...
- [x] **K-5 baseline realign + Operator populated coverage** (issue #396, 2026-07-24) — 내비게이션 아이콘 변경(#387/#388), OpenCode 제거(#392), 공유 Dropdown 통합(#395/#397) 이후 누락된 macOS baseline을 정렬했다. 갱신 전 28 PASS / 50 FAIL에서, deterministic Operator fixture 기반 카드 4개 + 상세 모달 2개 시나리오를 추가한 뒤 84/84 PASS로 전환했다. 이전 `#projects` 경로의 orphan baseline 4개를 제거하고, 갱신·신규 PNG 56개를 dark/light·desktop/mobile 축소 시트와 대표 원본으로 직접 검수했다.
