...
> **상태 (2026-05-04): READY r7; post-impl L9.1 verification stamp (2026-05-04). M4-a 머지 (PR #172) + 운영 가이드 (PR #176) + diagnose-mcp fix (PR #175) 후 §L9 R2 의 정적 도구 사용 핵심 가치 end-to-end 검증 완료. 동적 listChanged 전파는 미검증 (M4-c 후보). 이전: DRAFT r4 — Claude+Codex(gpt-5.5) cross-check 5회 정렬, r1→r7 까지의 변경 매트릭스는 §8 결정 출처 참고.**
>
...
| ⚠ 동적 워크스페이스 추가 시 *이미 떠있는* 워커가 listChanged 인지 | 미검증 | Streamable HTTP transport 의 server → client push 메커니즘 (SSE 채널 또는 long-poll) + codex 의 listener 구현 매트릭스 검증 영역. 사용자가 Bifrost 측에서 워크스페이스 추가/제거 시 검증 가능. 본 phase 의 핵심 가치 (정적 도구 사용) 와 무관 — 후속 phase 후보 (`M4-c` 가칭). |
| ⚠ 다른 서드파티 hosted MCP (Linear / Notion 공식 / Sentry) 의 동일 패턴 | 미검증 | 외부 endpoint 의 RFC 7231 비준수 패턴 발견 시 spec §L6 의 pass list 확장 또는 fallback (POST initialize ping) 도입 검토. M4-c 진입 트리거 중 하나. |

핵심 결론: **M4-a 의 `워커가 Bifrost 도구를 정적으로 호출 가능` 이라는 핵심 가치는 end-to-end 검증 완료**. 동적 tools/list_changed 전파는 본 phase 범위 외 (R2 의 "추가 가치" 라고 spec 이 명시) 로 유지.
...
- **M5 (가칭) — file-based MCP config delivery**: `flattenMcpToCodexArgs` 의 KNOWN LIMITATION (env values land in argv) 을 파일 기반 채널로 근본 해결. M4 완료 후 별도 phase. issue #113 참조.
- **`'sse'` transport** — SSE-only 원격 MCP 가 등장하거나 Bifrost 가 `/sse` 만 노출하는 운영 모드가 생길 때.
- **dynamic `tools/list_changed` 전파** — R2 검증에서 작동 안 한다고 판명되면 별도 phase.
