...
**Phase 2 — Worker 구조화 ✅ 완료·머지 (2026-07-16)**: 조사 결과 Phase 2 4개 항목 중 **Worker 구조화만 구현 가능**(나머지 3개는 소비자/CLI 부재로 defer/drop, 아래). Codex 설계 3R(GO-WITH-CHANGES ×2 → non-retryable 항목 claim-전 배치로 해소). 구현: migration **062**(`agent_profiles.model`/`reasoning_effort`, NULL=opt-out byte-identical) + `utils/agentVendor.resolveAgentVendor`(공용 command-based) + `agentProfileService.validateStructuredModelEffort`(create + update-**merged-state** PATCH, tokenizer 기반 double-set/vendor/leading-dash, `-m`이 `--permission-mode` 오탐 안 함) + lifecycleService **vendor-aware 주입**(codex extraArgs `structured→service_tier→baseArgs`, claude `spec.model`) + **claim-전 fail-closed non-retryable backstop**(`worker:profile_invalid`, started_at null → B-lite skip + goal isNonRetryable) + 워커 실행값 `runs.session_model/effort` snapshot + `AgentsView` 구조화 필드(vendor-aware, NULL 계약, 새 codex 기본=structured effort). **model_policies 테이블은 워커에 미적용**(워커=프로필 authoritative 원칙 유지). MP-1/2/3 각 codex-goal + Claude 리뷰 + 전체 회귀(2344) + a11y/visual 무회귀.
- **DEFER/DROP (Phase 2 스케치였으나 현 코드베이스에 소비자 없음 → dead code)**: claude thinking(streamJsonEngine이 thinking 플래그 미emit, API 파라미터라 CLI spawn 경로 없음), gemini vendor(manager 어댑터 팩토리=codex+claude만, operator 소비자 0; gemini 워커는 Worker 구조화로 커버), operator_profile 스코프(specialist=non-CLI direct backend, 주류 operator=profile_id NULL). 각 선행작업 생기면 재개.

**Phase 3 — Cost cap ✅ 완료·머지 (2026-07-16, Claude 구현 + Codex 검토)**: 두 항목 중 **cost cap만 구현**(node feasibility=워커 모델에러 경로가 executionEngine 출력 파싱이고 마진 낮아 defer). `projects.budget_usd`(기존 dormant 필드) + `runs.cost_usd`(추적됨)로 **spawn-time 예산 상한**: `runService.sumProjectCost`(task-linked runs 합산, 매니저 제외) + `spawnQueuedRun`이 claim 전 `spent >= budget_usd` → **reject**(`run:budget_exceeded` + failed, silent downgrade 없음). NULL=opt-out(byte-identical), 0/음수도 cap, lookup 에러 fail-open. **비-retryable 인프라 대폭 경화(Codex 5R)**: pre-claim 거부를 `runService.rejectQueuedRun` 원자 CAS(`WHERE status='queued'` 단일 UPDATE — status/retry_count `MAX`/`non_retryable=1`/`ended_at`)로 통일 + run:status/run:ended emit. migration **063** `runs.non_retryable`(durable) + `goalVerdictService.isNonRetryable = failed && (!started_at || non_retryable)` → goal retry child(goal_active=1) + started_at 보존 requeue 반례까지 error/non_retryable로 종결. **이 CAS는 P2 profile-invalid backstop도 공유**(같은 리뷰에서 findings 3+4 수정). Codex 최종 SHIP.
- Codex 5R: R1 FIX(2/3/4+새버그3) → R2 #4·새버그3 RESOLVED → R3 #2·#3 RESOLVED(non_retryable 컬럼) → R4 CAS 잔여(retry_count MAX·ended_at) → **R5 SHIP**.
