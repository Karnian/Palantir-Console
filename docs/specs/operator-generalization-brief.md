...
> **상태**: **v1.2 (2026-06-19) — O1~O15 전부 lock-in 완료, P-A1 착수 가능.** Codex R1+R2 적대검토 반영(R2 NO-GO 5건: §8 8~10·§4 ledger 3중계약·§13 흡수 matrix + lock-in O12~O15). **P-A0a(spawn-cwd 중앙화) merged #225**; A0b(capability scaffold)·virtual guard 는 P-B 폴드. 다음 = **P-A1**(owner-keying migration).
> **작성**: 2026-06-18
...
✅ **v1.2 — O1~O15 lock-in 완료 + P-A0a merged #225.** 다음 = **P-A1**(owner-keying migration): §10 의 5 slice(① schema nullable owner + backfill/dual-write/parity → ② L2 scope 제거 → ③ L1 owner 추상화 + 전 writer → ④ 3-part ledger + retrieve 시그니처 → ⑤ old API 제거 + invariant 테스트). partial 금지·dual-write rollout. 감독모드(Claude 계획→Codex 리뷰→Codex 구현→Claude 리뷰)로 slice별 진행.
