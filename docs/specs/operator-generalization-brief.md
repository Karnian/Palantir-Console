...
> **상태**: **v1.2 (2026-06-19) — O1~O15 전부 lock-in 완료.** Codex R1+R2 적대검토 반영(R2 NO-GO 5건: §8 8~10·§4 ledger 3중계약·§13 흡수 matrix + lock-in O12~O15). **P-A0a(spawn-cwd 중앙화) merged #225**; A0b(capability scaffold)·virtual guard 는 P-B 폴드. **P-A1 owner-keying STORAGE 완료(#227~#231; L1+L2 items+candidates+scheduler). 잔여=slice4(→P-A2 Composer)·slice5 cleanup·P-B(Profile FK).** 다음 = **P-A2**(Memory Composer) 또는 **slice5 cleanup**(old unique 인덱스 drop + resolveOwnerFromProject 제거 + 교차오염 음성 테스트).
> **작성**: 2026-06-18
...
> **Note**: manager-slot compositions use (run, conversation) identity; `task_id=null` is by-design (manager runs are not task-bound). See `compositionLedger.js` header.
...
> **✅ LOCK-IN 완료 (2026-06-19) — O1~O15 전부 확정**: O1 MVP(coder 불변+specialist stateless) · O2 in-memory virtual · O3 `adapter_config`=owner-keying PR 과 함께 · O4 archive 제외 · O5 A=owner-keying 부터 · **O6 coder=Workspace-only**(User 후속) · O7 최소분리 · **O8 Master=singleton** · O9 `workspace=none`=내부 metadata 검색만 · O10 비영속 · O11 route/provenance · **O12 복합키 `(owner_type,owner_id)` + `normalizeOwner()`**(FK 유지, owner_id 단독비교 금지) · **O13 constraint>fact>heuristic + mode-aware** · **O14 revision-only + 불변 edge ledger**(재검색 `status='active'` 자동배제 → edge mutate 불필요) · **O15 dual-write+flag · write-authority 예약 · cross-owner raw inspect ✕**.
> → **P-A1(owner-keying) 착수 가능** — schema·3-part ledger·Composer 설계가 완전 정의됨.

---

## 10. PR breakdown (lock-in 후, 각 PR Codex 교차검증)

- **P-A0** — ✅ **A0a(spawn-cwd 중앙 helper) merged #225** (`resolveSpawnCwd`, 7경로, no-dir 정책 보존, `requireExplicit` seam 미와이어). **A0b(capability deny scaffold) + virtual 배제 가드 → P-B 폴드** (미와이어 scaffold·virtual guard 는 consumer(specialist) 생길 때).
...
- **✅ STORAGE owner-keying 완료** (slices merged #227~#231; gate=provenance-keyed 의도 분리).
  1. ✅ **#227 (slice 1)** schema = nullable owner_type/owner_id + 배치 backfill + dual-write + parity/detectCrossScopeConflicts gate (9 테이블).
  2. ✅ **L2 atomic cutover — 재배치(Codex 적대판정): 원안 "2 L2 scope 제거 → 5 owner-unique" 는 순서역전(owner-keyed reads ⟸ owner-unique ⟸ cross-scope merge 선행 필요; L2 단일 owner) → 2 sub-PR**:
     - ✅ **#228 (2a)** L2 cross-scope dedup merge(winner=human권위(O13)/pinned·human 절대보호 우선 + 보호 상속) + owner-unique 인덱스(items content_hash/fact_key, candidates rule/dedup_key); migration 034 tx 원자(merge+DDL) + fail-closed verify.
     - ✅ **#229 (2b)** L2 storage owner cutover(reads/dedup/merge/cap(owner 1000)/admission/eviction/retrieve+provenance owner-keyed; write merge 로 cross-scope 충돌 해소). **핵심: revision/injection ledger = scope(provenance)-keyed 유지** — owner 통합은 cross_project 쓰기가 user 주입 재트리거(BLOCKER, Codex 가 잡아 revert). **불변식: storage=owner-keyed / 주입게이트=provenance-keyed.**
  3. ✅ **#230 (slice 3)** L1 project memory storage owner cutover(memory_items reads/dedup/merge/cap/eviction owner-keyed; A1 two-pass·human/pinned 보호 보존; behavior-preserving owner_id=project_id) + migration 036 memory_jobs owner single-flight.
  3b. ✅ **#231 (slice 3b, 사후 plan-review 가 잡은 partial 보강)** L1 candidates owner-unique(migration 037) + createCandidate owner-keyed dedup(non-object 검증 사전조회前=CHECK surface + insert try/catch race-safe) + distill 스케줄러 owner-enumeration(non-workspace skip=P-B 연기).
  4. **(남음 → P-A2 Composer 영역)** injection ledger §4 3-part(composition fingerprint + owner revision vector + per-item edge) + **owner-native 공개 API**. (retrieve 시그니처는 2b/3 에서 이미 owner-keyed 전환됨.)
  5. **(남음)** old project/scope unique 인덱스 drop(parity 후) + `resolveOwnerFromProject`/legacy 래퍼 제거 + invariant 교차오염 음성 테스트.
  - **deferred 제품결정**: cross_project→Top 주입(현재 미주입 보존; 활성화 시 injection ledger 재검토). **P-B**: `memory_jobs.project_id NOT NULL FK` 완화(Profile job) + Profile candidate 생성경로.
...
✅ **v1.2 — O1~O15 lock-in 완료 + P-A0a merged #225 + P-A1 STORAGE owner-keying 완료(#227~#231).** P-A1 완료 내역: slice1(schema dual-write #227) → slice2a(L2 cross-scope merge+owner-unique #228) → slice2b(L2 storage cutover #229) → slice3(L1 memory storage cutover #230) → slice3b(L1 candidates+scheduler #231). **불변식 확립: storage=owner-keyed / 주입게이트=provenance-keyed.** 잔여 = **slice4**(→P-A2 Composer 영역: 3-part ledger + owner-native API) + **slice5 cleanup**(old unique 인덱스 drop + `resolveOwnerFromProject`/legacy 래퍼 제거 + invariant 교차오염 음성 테스트) + **P-B**(Profile FK 완화 + Profile candidate 생성경로). 다음 우선순위:
- **slice5 cleanup** — old project/scope 인덱스 drop + resolveOwnerFromProject 제거 + 교차오염 음성 테스트(parity 통과 후 즉시 진행 가능).
- **P-A2 Memory Composer** — 단일주입 + 다중 owner 예산 + 3-part injection ledger(composition fingerprint + owner revision vector + per-item edge) + conversationService 직접 prepend 흡수(parent-notice 계약 분리). memory-augmentation A2-④(극성 게이트)는 P-A1 write 경로에.
감독모드(Claude 계획→Codex 리뷰→Codex 구현→Claude 리뷰)로 slice/PR별 진행.
