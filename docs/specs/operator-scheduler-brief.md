...
- scheduled codebase turn의 `codebase_project_id`는 생성·수정·실행 시 존재 검증한다. 일반 대화의
  ...
  primary/reference로 매핑된 folder만 허용한다. 또한 target folder는 Operator primary와 같은 node에
  있어야 한다. 삭제/비활성/교차-node folder는 fail-closed한다.

## 5. 스키마

실제 MVP 스키마는 `server/db/migrations/067_operator_scheduler.sql`이며, 적대 리뷰 하드닝은
`068_operator_scheduler_hardening.sql`이다. 아래 r1 설계안에서 MVP는
사용자 lock-in에 맞춰 `codebase` turn만 열었고, `generic`/event source, 명시적 skip history,
failed/uncertain retry API는 후속으로 남겼다. 중복·overlap은 schedule 회차 unique와 Operator 단위
active partial unique index, 최신 회차 coalesce로 강제한다.
...
1. **Turn completion correlation — 해결**: Claude/Codex terminal normalized event에 `invocationId`와
   `terminal=true`를 기록하고, durable event id로 payload를 재조회한 뒤 invocation id + manager run id가
   모두 일치할 때만 완료한다. Codex의 중간 `item.type=error`는 `terminal=false`라서 완료를 선점하지 않는다.
