...
### ~~US-007: Preset 격리 검증 테스트 (Non-functional)~~ — 제외 (Phase 10G 종결)
> end-to-end host canary listing 검증은 실제 Claude CLI 가 spawn 되어야 하므로 단위 테스트에서는 불가. argv-level Tier 2 wiring 검증 (`preset-isolated.test.js`) 으로 G1 보장은 충분하다고 판단하고 자동화 시나리오에서 제외. 운영자가 isolated 워커 사용 시 수동으로 호스트 plugin 노출 확인.
...
const presetJson = JSON.stringify({
  name, description, isolated, plugin_refs, mcp_server_ids, base_system_prompt, setting_sources
});
const fileHashes = [];
...
// computePresetDrift 반환 shape
{
  ...
  drift_error: 'string',    // optional — present when file hash recomputation failed
}
```
...
- `has_drift = changed_fields.length > 0 || changed_files.length > 0`. `drift_error` 단독으로는 `has_drift` 를 true 로 만들지 않는다.
- UI: RunInspector Preset 탭의 배지 (`⚠ N`) 는 `changed_fields + changed_files` 합산. `changed_files` 는 path, status 를 목록으로 표시.

**Backward compatibility (Phase D)**: Phase D 이전에 생성된 legacy snapshot_json row 는 `description` 필드를 포함하지 않을 수 있다. Drift 비교는 snapshotCore 에 실제로 존재하는 필드만 비교(hasOwnProperty 가드)하므로 legacy run 에서 description 이 spuriously changed 로 표시되지 않는다. 과거 snapshot_json row 는 backfill 하지 않음 — forensic history 보존 원칙.

**Degraded mode (drift_error)**: `computeCurrentFileHashes` 가 throw 하면 API 는 200 + `drift.drift_error` 로 응답한다. `changed_files` 는 빈 배열을 유지한다. UI 는 `drift_error` 상태에서 초록 "matches snapshot" 배너를 절대 띄우지 않음.
...
- ~~격리 canary 테스트 (US-007)~~ — Phase 10G 에서 제외
...
- [—] **US-007 — EXCLUDED**. End-to-end host plugin listing 검증은 실제 Claude CLI spawn 이 필요해 CI 자동화 범위에서 제외. argv-level Tier 2 wiring (`preset-isolated.test.js` 의 `--bare`/`--strict-mcp-config`/`--setting-sources`/`--plugin-dir` 검증) 으로 G1 보장 충분으로 판단. 운영자 수동 검증으로 대체.
...
## 14. Historical Addenda

### 2026-07-23 — OpenCode supported-vendor removal

OpenCode support was later removed as a supported agent vendor by the OpenCode vendor-removal effort (Phases 1–5). Accordingly, G4's OpenCode-specific clause no longer applies. The original specification and round-by-round review trail above remain unchanged as historical records.
