...
- **Tier 1 (portable schema, adapter-specific wiring)**: MCP 서버 정의 + 시스템 프롬프트. 주입 경로는 어댑터별로 **정확히 지정됨**:
  - Claude: `streamJsonEngine.startSession` 의 `systemPrompt` + `mcpConfig` 파라미터 (현재도 사용 중인 경로)
  - Codex: `codex exec` 의 `-c 'mcp_servers=<json>'` config override + `{system_prompt_file}` placeholder 파일 치환 (Phase 10C 에서 구현)
  - OpenCode: `opencode run --mcp-config <file>` (있으면) 또는 env var 기반 override + `{system_prompt_file}` placeholder. OpenCode 가 MCP 주입을 지원하지 않으면 warning + prompt 만 적용 (graceful degrade, Phase 10C 에서 실측 후 확정).
  - **즉 Tier 1 은 "같은 스키마"를 주지만 "같은 주입 경로"는 아니다**.
...
| G3 | 기존 Skill Pack 비파괴 | 기존 651+ 테스트 회귀 0건 |
...
CREATE TABLE run_preset_snapshots (
  ...
  preset_snapshot_hash TEXT NOT NULL,                -- SHA-256 of full preset + file hashes
  snapshot_json TEXT NOT NULL,                       -- frozen preset content
...
Isolated preset (`isolated: true`) spawn 시 Claude auth 결정:
```
1. If env ANTHROPIC_API_KEY is set and non-empty  → pass through via spawn env
2. Else if authResolver reads valid token from
   a. ANTHROPIC_API_KEY env (redundant check)
   b. CLAUDE_CODE_OAUTH_TOKEN from .claude-auth.json
   c. macOS keychain (via `security find-generic-password`)
   → inject CLAUDE_CODE_OAUTH_TOKEN env (keychain case: extract token first)
3. Else → fail-closed: 400 "Isolated preset requires Claude auth.
   Set ANTHROPIC_API_KEY, run Palantir from a Claude Code session,
   or store token in keychain."
```

`.claude-auth.json` 은 sources 중 하나일 뿐. 부재 자체가 reject 이유가 아님.
...
- `lifecycleService` + `executionEngine` 에 preset MCP config 주입:
  - Claude: `streamJsonEngine.startSession({ systemPrompt, mcpConfig })` (이미 사용 중)
  - Codex: `codex exec -c 'mcp_servers=<json>' ...` + prompt placeholder 파일
  - OpenCode: `opencode run` 실측 — MCP config flag 존재 확인. 없으면 prompt-only + `preset:mcp_unsupported` warning
...
| 삭제된 preset 을 참조하는 past run | LOW | snapshot 은 cascade delete 안 걸리도록 FK NULL. 과거 run 은 snapshot 만으로 설명 가능. |
...
> "No remaining spec issues found in `docs/specs/worker-preset-and-plugin-injection.md`. Round 2 fixes close the two material gaps... Internal consistency across §6.3, §6.8, and Phase 10C now holds."

Spec v0.1 수렴 완료. **Status: Approved — Phase 10A 착수 가능.**
