...
-- 045_operator_rename_persisted.sql
-- Phase 1: relax runs.manager_layer and memory_composition_events.slot_kind CHECKs
-- to accept 'operator', then rewrite persisted 'pm:' / 'pm' values.
--
-- Dual-read (Phase 0) is already live: all consumers accept both forms.
-- Producers still write 'pm' / 'pm:' until they flip in Phase 2.
--
...
-- Step 1a: Rebuild runs with relaxed manager_layer CHECK.
CREATE TABLE runs_new (
  ...
  manager_layer        TEXT    CHECK(manager_layer IS NULL OR manager_layer IN ('top','pm','operator')),
...
-- Step 1b: Rebuild memory_composition_events with relaxed slot_kind CHECK.
CREATE TABLE memory_composition_events_new (
  id                   TEXT    PRIMARY KEY,
  run_id               TEXT    NOT NULL,
  conversation_id      TEXT,
  task_id              TEXT,
  slot_kind            TEXT    NOT NULL  CHECK(slot_kind IN ('top','pm','operator')),
...
-- Step 2: Data migration. CHECKs now allow 'operator'.
