...
function createLifecycleService({
  ...
  function buildCodexWorkerInvocation(profile, prompt, placeholders = {}) {
    ...
    // A legacy profile may consist of `{prompt}` alone. Moving that marker to
    // stdin must also supply Codex's non-interactive subcommand; `codex -`
    // would otherwise enter the wrong CLI mode instead of running the task.
    if (args.length === 0) {
      return { args: ['exec', '-'], stdin: prompt };
    }
