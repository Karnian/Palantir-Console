...
function renderSpecialistInvokePanel(env, props = {}) {
  const root = env.document.getElementById('root');
  env.render(env.h(env.context.SpecialistInvokePanel, {
    runs: [],
    ...props,
  }), root);
  return root;
...
test('SpecialistInvokePanel seeds initialProfileId after profiles load and auto-selects the only active manager run', async (t) => {
...
test('SpecialistInvokePanel ignores an initialProfileId that is not in the loaded profile list', async (t) => {
  const env = createPreactEnv();
  t.after(env.cleanup);
...
test('SpecialistInvokePanel drops the auto-selected origin run when the active set grows to multiple (forces explicit pick)', async (t) => {
  const env = createPreactEnv();
  t.after(env.cleanup);
  ...
  renderSpecialistInvokePanel(env, {
    initialProfileId: 'op_review',
...
test('SpecialistInvokePanel with zero active manager runs keeps submit disabled and shows the origin hint', async (t) => {
  const env = createPreactEnv();
  t.after(env.cleanup);
...
test('SpecialistView is a thin page wrapper around SpecialistInvokePanel', async (t) => {
  const env = createPreactEnv();
  t.after(env.cleanup);

  env.context.SpecialistInvokePanel = function SpecialistInvokePanel({ initialProfileId, runs }) {
    return env.context.preact.h('div', {
      'data-role': 'specialist-invoke-panel-stub',
      'data-initial-profile-id': initialProfileId,
  ...
  const root = renderSpecialistView(env, {
    initialProfileId: 'op_review',
    runs: [{ id: 'run-one', is_manager: 1, status: 'running' }],
  });

  const panel = root.querySelector('[data-role="specialist-invoke-panel-stub"]');
  assert.ok(panel);
  assert.match(root.textContent, /스페셜리스트/);
  assert.equal(panel.getAttribute('data-initial-profile-id'), 'op_review');
  assert.equal(panel.getAttribute('data-runs'), '1');
});
