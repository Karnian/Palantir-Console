...
describe('worktreeService auto-save on removal', () => {
  ...
  it('auto-commits uncommitted changes before removing worktree', async () => {
    const { path: wtPath, branch } = await ws.createWorktree(repoDir, 'palantir/auto-save-test');
    ...
    assert.ok(await ws.hasUncommittedChanges(wtPath), 'should detect uncommitted changes');

    // Remove worktree — should auto-save first
    await ws.removeWorktree(repoDir, wtPath, branch, { runId: 'test-run-1' });
    ...
    assert.ok(await ws.branchHasWork(repoDir, branch), 'branch should have work');
  ...
  it('deletes branch when worktree has no changes', async () => {
    const { path: wtPath, branch } = await ws.createWorktree(repoDir, 'palantir/clean-wt-test');
    assert.ok(fs.existsSync(wtPath), 'worktree should be created');

    // No changes made — remove should delete the branch
    await ws.removeWorktree(repoDir, wtPath, branch, { runId: 'test-run-2' });
  ...
  it('preserves branch when worker committed changes (no auto-save needed)', async () => {
    const { path: wtPath, branch } = await ws.createWorktree(repoDir, 'palantir/committed-test');
    ...
    assert.ok(!await ws.hasUncommittedChanges(wtPath), 'should have no uncommitted changes');

    // Remove worktree
    await ws.removeWorktree(repoDir, wtPath, branch, { runId: 'test-run-3' });

    // Branch should be preserved (has commits)
    assert.ok(await ws.branchHasWork(repoDir, branch), 'branch should be preserved');

    // Cleanup
    try { execFileSync('git', ['branch', '-D', branch], { cwd: repoDir, stdio: 'pipe' }); } catch { /* ok */ }
  });

  it('hasUncommittedChanges returns false for clean worktree', async () => {
    const { path: wtPath, branch } = await ws.createWorktree(repoDir, 'palantir/clean-check');
    assert.ok(!await ws.hasUncommittedChanges(wtPath), 'clean worktree should have no uncommitted changes');
    await ws.removeWorktree(repoDir, wtPath, branch);
  });
});
